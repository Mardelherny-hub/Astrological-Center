...
(function(app) {
	...
	app : {
		init : function(callback) {
			var params = {
				data:{active:true},
				url : '../services/getFrontSite/index.php',
		...
		getMenuArticles : function(parametros,callback){
			parametros.active = true;
			var params = {
					url : '../services/getMenuArticles/index.php',
					data: parametros,
					success : function(data) {
						callback(app.data.PersistentData.parse(data));
					}
		...
		getArticles : function(parametros,callback){
			parametros.active = true;
			var params = {
					url : '../services/content/getFrontContents/index.php',
					data: parametros,
					success : function(data) {
						callback(app.data.PersistentData.parse(data));
					}
		...
		getContent : function(parametros,callback){
			parametros.active = true;
			var params = {
					url : '../services/content/getFrontContent/index.php',
					data: parametros,
					success : function(data) {
						callback(app.data.PersistentData.parse(data));
					}
...
// start -> ../sources/app/view/menu.js **************************************************
app.view.Menu = app.View.extend({
	...
	render:function(){
		...
		var ul;
		var a;
		var menu = $('<ul class="list-unstyled list-inline">');
		$(this.el).append(menu);
		...
		for(var i=0;i<site.length;i++)
		{
			...
			if(s.details.toMainMenu==1)
			{
				...
				if(s.sections.length)
				{
					...
					{
						subs = s.sections[j];
						li2 = $('<li class="mainMenuBtn">');
						ul.append(li2);
						a = $('<a>');
						a.attr('href',Utils.resolveRef(subs.type,subs.path));
						a.append(subs.details.name);
						li2.append(a);
						li2.attr('id',subs.furl);
					}
					//para css
				 li.attr('h',ul.height());
				 log(ul.height(),ul.width());
				 ul.height(0);
				 li.hover(function(){
					//console.log('here');
					var ul = $(this).find('ul');
	...
	updateSubActiva : function(){
		if(this.currentSub)this.currentSub.removeClass('active');
		var current = this.model.get('subActiva');
		this.currentSub = $(this.el).find('li#'+current);
		this.currentSub.addClass('active');
	}
});
// end -> ../sources/app/view/menu.js **************************************************

// start -> ../sources/app/view/menuFooter.js **************************************************
app.view.MenuFooter = app.View.extend({
	el:'.footer .linksFooter',
	render:function(){
		var ui = $(this.el);
		ui.empty();		
		var site = this.model.get('site');
		...
		{
			s = site[i];
			ui.append('<a href="'+Utils.resolveRef(s.type,s.path)+'">'+s.details.name+'</a>');	
			if(s != site[site.length-1])ui.append(' | ');					
		}
...
// end -> ../sources/app/view/menuFooter.js **************************************************

// start -> ../sources/app/view/menuIdiomas.js **************************************************
app.view.menuIdiomas = app.View.extend({
	...
	render:function(){
		...
		log("IDIOMAS",idiomas);
		var menus = $(this.el).find('.menuIdiomas');
		...
		menus.each(function(){
			...
			for(var i=0;i<idiomas.length;i++)
			{
				...
				var id = idiomas[i];
				$(this).append(' <li><a href="#" class="btnIdioma" idi="'+id.idioma+'">'+id.nombre+'</a></li>');
			}
		});
...
// end -> ../sources/app/view/menuIdiomas.js **************************************************

// start -> ../sources/app/view/menuLateral.js **************************************************
app.view.MenuLateral = app.View.extend({
	...
	initialize: function() {
	    ...
	    this.listenTo(this.model, "change:seccionActiva", this.updateSectionActiva);
	    this.listenTo(this.model, "change:subActiva", this.updateSubActiva);
	    this.listenTo(this.model, "change:currentContents", this.updateCurrentContents);
	    this.listenTo(this.model, "change:contentActive", this.updateContentActive);
	},
	updateContentActive : function(){	
		if(this.contentActive)this.contentActive.removeClass('active');
		
		var contentActive = this.model.get('contentActive');
		this.contentActive = $(this.el).find('li#'+contentActive)
		this.contentActive.addClass('active');
	},
	updateCurrentContents : function(){		
		...
		for(key in contents.contents){
			...
			li.append(d);
			a = $('<a>');
			a.attr('href',Utils.resolveRef(content.type,content.path));
	...
	updateSubActiva : function(){
		...
		this.currentSub.addClass('active');
		log('UPDATE SUB',this.currentSub);
	}
});
// end -> ../sources/app/view/menuLateral.js **************************************************

// start -> ../sources/app/view/newsletter.js **************************************************
app.view.Newsletter = app.View.extend({
	el:'div#formNewsletter',
	initialize: function() {
		var ui = $(this.el);
	    $(this.el).find('#btnNL').click(function(e){
	    	e.preventDefault();

	    	var n = ui.find('#inputnombre');
	    	var e = ui.find("#inputemail");

	    	newsletterForm = {nombre:n.val(),email:e.val()};

	    	Url.setHash('#/contact/news/?n=1&t=' + Utils.timestp());
...
var newsletterForm;
// end -> ../sources/app/view/newsletter.js **************************************************

// start -> ../sources/app/view/sections/app.Articulo.js **************************************************
app.view.sections.Articulo = app.Section.extend({
	el:'#articulos-section',
	articulo:null,//ui
	contenedor:null,//ui
	initialize: function(){

		this.model = new app.data.models.articleModel();
		this.model.onRegister();

		this.articulo = $(this.el).find('.articulo');
		this.contenedor = $(this.el).children('#artContainer');
	...
	renderArt:function(){
		...
		var data = this.model.get('contenidosView');
		this.contenedor.empty();
		...
		if(data.length>1){
			short = true;
			for (key in data){
					data[key].datatext = Utils.shortArticle(data[key].details.descripcion);
				}
		}

		for (key in data){
			dataArticulo = data[key];
			var art = this.articulo.clone();
			this.contenedor.append(art);
			//console.log(dataArticulo);
			art.children('.artTitulo').children('.inner').text(dataArticulo.details.titulo);
			//art.children('.artTitulo').children('.inner').fitText(1.2, { minFontSize: '20px', maxFontSize: '40px' })
			var url = "http://www.cgjung.com.ar/aztlanback/es/" + dataArticulo.path;
			art.find('.fb-like').attr('data-href',url);
			var c = art.children('.artContenido');
			var p = $('<p>');
			if(short){
			...
			else p.html(dataArticulo.details.descripcion);
			if(dataArticulo.img && dataArticulo.img !=''){
				var path = "../images/images/" + dataArticulo.img;
				var clazz = Utils.resolveImgAlign(dataArticulo.imgAlign);
				var img = ('<img src="'+path+'" class="'+clazz+'">');

				//si es centrada...agregamos un p
				if(dataArticulo.imgAlign == 1){//img-center
					var np = $('<p>');
					np.append(img);
					np.addClass(clazz);
					c.append(np);
					c.append(p);
				}//si es abajo, agregamos p abajo
				else if(dataArticulo.imgAlign == 3){
					var np = $('<p>');
					np.append(img);
					np.addClass(clazz);
					c.append(p);//primero el p, luego el p de imagen
					c.append(np);
				}else if(dataArticulo.imgAlign == 4 || dataArticulo.imgAlign == 5){//super-left
					art.prepend(img);
					c.append(p);
			...
			log("S",dataArticulo.datatext,short);
			...
			//var mas = art.find('.verMas').remove();
			//c.append(mas);
		}

		//div.append("A: " + this.model.get('articles'));
		if(hasFacebook && hasFacebook == false)log('FB doNothing');
		else FB.XFBML.parse();
	},
	...
	renderSeccion:function(){
		...
		var ui =  $(this.el).children('.branch').find('span.bsection');
		var a = $('<a>');
		a.text(seccion.details.name);
		var href = Utils.resolveRef(seccion.type,seccion.path);
		a.attr('href',href);
		ui.html(a);
	},
	renderParent:function(){
		...
		var ui =  $(this.el).children('.branch').find('span.bparent');
		ui.empty();
		if(!seccion)return;
		var a = $('<a>');
		a.text(seccion.details.name);
...
// end -> ../sources/app/view/sections/app.Articulo.js **************************************************

// start -> ../sources/app/view/sections/app.Camp.js **************************************************
...
// end -> ../sources/app/view/sections/app.Camp.js **************************************************

// start -> ../sources/app/view/sections/app.Contacto.js **************************************************
app.view.sections.Contacto = app.Section.extend({
	...
	initialize: function(){
		...
		this.model = new app.data.models.contactoModel();
		this.model.onRegister();
		this.listenTo(this.model, "change:response", this.onResponse);
    ...
    this.facebook = ui.find('input#inputFacebook');
		...
		this.consultaOk = ui.find('.consultaOk');
		this.error = ui.find('.errorMsg');
		this.newsOk = ui.find('.newsOk');

		ui.find("form").validate(Validation.validate(ui.find("form"),function(){
			//log("FORM",form);
        	var params = {};
        	params.newsletter = (self.isNews) ?1:0;
          ...
          params.curso = self.curso.val();
        	params.consulta = self.consulta.val();

        	self.model.setConsulta(params);
		}));
	},
	onResponse:function(){
	...
	show : function(){
		...
		this.isNews = false;
		var params = this.model.get('query');
		params = params.params;
		log('SHOW CONTACTO',params);
		var ui = $(this.el);

		this.consultaOk.hide();
		this.newsOk.hide();
		this.error.hide();

		ui.find("input").val('');
		ui.find("textarea").val('');
		ui.find(".error").html('');
		ui.find(".error").removeClass("error");
		ui.find('.has-error').removeClass('has-error');
		ui.find('.has-success').removeClass('has-success');


		//log("PARAMS",params);
		if(params && params.n=="1" && newsletterForm && newsletterForm.nombre && newsletterForm.email ){
			ui.find('.hnews').show();
			ui.find('.hcontactenos').hide();
			this.isNews = true;
			this.name.val(newsletterForm.nombre);
			this.email.val(newsletterForm.email);
			newsletterForm = null;
		}
		else{
			ui.find('.hnews').hide();
			ui.find('.hcontactenos').show();
		}

		ui.find("form").show();
		ui.show();

		this.stopListening(this.model, "change:query");
		this.listenTo(this.model, "change:query", function(){
			if($('div#contacto-section:visible'))this.show();
		});
...
// end -> ../sources/app/view/sections/app.Contacto.js **************************************************

// start -> ../sources/app/view/sections/app.Error.js **************************************************
...
// end -> ../sources/app/view/sections/app.Error.js **************************************************

// start -> ../sources/app/view/sections/app.Home.js **************************************************
app.view.sections.Home = app.Section.extend({

	el:'#home-section',
	hasFlash : false,
	initialize : function(){
		//addFlash("flashHome",'swf/banner.swf');
		//console.log('h int');
		this.hide();
	},
	resolveHash : function(route) {
	...
	hide : function(){
		//console.log('HIDE');
		$('#contents').removeClass('inHome');
		$('div.ciz').height($('#contents').height());
		$(this.el).hide();
	},
	...
	show : function(){
		if(!this.hasFlash)
			{
				addFlash("flashHome",'../site/swf/banner.swf');
				this.hasFlash = true;
			}
...
// end -> ../sources/app/view/sections/app.Home.js **************************************************

// start -> ../sources/app/view/sections/app.Videos.js **************************************************
...
// end -> ../sources/app/view/sections/app.Videos.js **************************************************

// start -> ../sources/app/view/popup/campPopupForms.js **************************************************
app.view.campPopupForms = app.View.extend({
	...
	loadPopup : function(){
		//console.log('HERE');
		var self = this;
		$( this.el).load( "popup/load.php?file=popupforms",function(){
			self.onLoaded($('#formsModal'));
		});
	},
	onLoaded:function($obj){
		var camp = this.model.get('camp');
		$obj.find('.titulo').text(camp.titulo);
		$obj.find('.fecha').text(camp.fecha);
		$obj.find('.panel-body').html(Utils.db64ToText(camp.texto));
		var self = this;
		var ui = $(this.el);
		...
		ui.find('button.close').click(function(){		
...
// end -> ../sources/app/view/popup/campPopupForms.js **************************************************

// start -> ../sources/app/view/popup/popup.js **************************************************
/*
...
*/
// end -> ../sources/app/view/popup/popup.js **************************************************

// start -> ../sources/app/model/articleModel.js **************************************************
app.data.models.articleModel = app.models.SectionModel.extend({
		name:'articleModel',
		seccion:'',
		parent:'',
		forClean:null,
	    ...
	    resolveQuery : function(query) {
			...
			this.set('forClean',Utils.timestp());//limpia contenedor
			...
			if(query.params){				
				app.logger.log('app.articleModel:resolveParams() ', query.params);				
				
				this.url.app.getContent({idContent:query.params.o,idSection:seccion.id},function(data){
					$self.set('seccion',data.section);
					$self.set('parent',data.parent);
					$self.set('contenidos',[data.content]);
			...
			app.logger.log('app.articleModel:resolveHash() ', query.route);	
...
// end -> ../sources/app/model/articleModel.js **************************************************

// start -> ../sources/app/model/campModel.js **************************************************
...
// end -> ../sources/app/model/campModel.js **************************************************

// start -> ../sources/app/model/contactoModel.js **************************************************
...
// end -> ../sources/app/model/contactoModel.js **************************************************

// start -> ../sources/app/model/idiomaModel.js **************************************************
...
// end -> ../sources/app/model/idiomaModel.js **************************************************

// start -> ../sources/app/model/scrollModel.js **************************************************
app.data.models.scrollModel = app.models.HashModel.extend({
		name: 'scrollModel',		
        initialize : function(){
        	this.register();
        },
    	hashChange: function(section,route,params){
    		...
    		else*/ bodyelem = $("html,body,document");
    		
    	app.logger.log("scrollModel hashChange",section,route,params,$(window).scrollTop() );
...
// end -> ../sources/app/model/scrollModel.js **************************************************

// start -> ../sources/app/model/siteModel.js **************************************************
app.data.models.siteModel = app.models.HashModel.extend({
    	...
    	hashChange: function(section,route,params){
    		//app.logger.log("MENU hashChange,update");
    		var self = this;
    		var seccion = section;
    		var sub = route[0];
    		if(seccion == 'article' || seccion == 'videos')
    			{
    				seccion = route[0];
    				sub = route[1];
    			}

    		var section = this.getIdRoutes(route);
    		var idcontent;
    		log("SECTION",section,this.get('subActiva'));
    		//si la subseccion no cambio, solo actualizo parametros
    		if(section && section.showContent==1 && this.sectionShowContent == null || section && section.showContent==1 && this.sectionShowContent.id != section.id)
    		{
    			self.url.app.getMenuArticles({idSection:section.id},function(data){
    				self.set('seccionActiva',seccion);
    				self.set('subActiva',sub);
    				self.set('currentContents',{id:section.furl,contents:data.contents})
    				idcontent = (params && params.n) ? params.n : null;
    				self.set('contentActive',idcontent);
    		...
    		this.sectionShowContent = section;
    	},
    	loadSite : function(){
    		var $self = this;
    	...
    	getIdRoutes : function(route)
    	{
    		...
    		/*
    		var obj = {seccion:seccion.id}
    		//console.log("A",articulo);
    		if(articulo) obj.subseccion = articulo.id;
    		if(route.length>2)obj.articulo = route[2];

    		return obj;
    		*/
    	}
});
// end -> ../sources/app/model/siteModel.js **************************************************

// start -> ../sources/app/model/videosModel.js **************************************************
...
// end -> ../sources/app/model/videosModel.js **************************************************

// start -> ../sources/app/model/popup/campPopupModel.js **************************************************
app.data.models.campPopupModel = app.models.ModalModel.extend({
    	...
    	showTrueAlert:function(){
    		...
    		//var camp = this.get('camp');
    		toastr.success("Tu mensaje fue enviado", "Gracias por contactarte con nosotros.");
    	},
		onModalClosed : function(){
			//Url.setHash('#/home');
		}
...
// end -> ../sources/app/model/popup/campPopupModel.js **************************************************

// start -> ../sources/app/model/popup/popupModel.js **************************************************
app.data.models.popupModel = app.models.ModalModel.extend({
	    ...
	    hashChange: function(section,route){
	    	if(section == "c")this.unregister();
	    	
	    	if(this.get('opened') == true){
	    		this.closepopup();
			}
	    	else if(section != "c"){//si no es campa?
	    		this.load();
	    	}
...
// end -> ../sources/app/model/popup/popupModel.js **************************************************
...
$(function() {
	...
	//Componentes solos
	var newsletter = new app.view.Newsletter();
	var idiomas = new app.view.menuIdiomas();
	...
	sectionManagerModel.registerSections(new app.view.sections.Videos(), 'videos');
	sectionManagerModel.registerSections(new app.view.sections.Contacto(), 'contact');
	...
	var menuFooter = new app.view.MenuFooter({model:siteModel});
	var scrollModel = new app.data.models.scrollModel();
	var popup = new app.view.Popup();
	//app.Address.register(siteModel);

	siteModel.bind('change',startSite);
	siteModel.loadSite();

	//siteModel.set('serviced','theData');
});
...
function checkCampaign(){
	...
	/*var camp = Utils.urlParams();
	if(camp && camp.cid && camp.so){
		var campforms = new app.view.campaingForms();
		campforms.model.getCamp(camp);
		return true
		//Url.setHash('');
